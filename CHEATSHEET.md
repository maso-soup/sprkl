...
| 4 | `forced-browsing-reports` | Forced browsing to unlinked corporate reports | Forced Browsing | ② | corporate | Force-browse /admin/reports/financials | GET /admin/reports/financials | canary | A01:2021 | CWE-425 CWE-862 | `forced-browsing` |
...
| 8 | `path-normalization-admin` | Path normalization bypass reaches admin console | Path Normalization | ⑤ | corporate | Force-browse /admin/public/..%2fadmin/console | GET /admin/public/..%2fadmin/console | canary | A01:2021 A05:2021 | CWE-22 CWE-436 | `path-normalization-bypass` |
...
| 7 | `mfa-bypass-skip-step` | Corporate MFA step is skippable | MFA Bypass | ④ | corporate | Admin sign-in → skip the OTP step | POST /admin/login then /admin/dashboard | sink-predicate | A07:2021 | CWE-287 CWE-308 | `auth-mfa-bypass` |
...
| 10 | `saml-signature-bypass` | SAML response signature not verified *(N/A)* | SAML | — | corporate | n/a | POST /admin/saml/acs | N/A | A07:2021 | CWE-347 CWE-290 | `auth-saml-attacks` |
...
| 7 | `nosql-login-bypass` | NoSQL operator injection auth bypass (corporate) | NoSQL Injection | ③ | corporate | Admin JSON login /admin/api/login | POST /admin/login (JSON body) | sink-predicate | A03:2021 | CWE-943 | `nosql-injection` |
...
| 9 | `os-command-injection` | OS command injection in label/invoice generator | OS Command Injection | ④ | corporate | Admin → Tools → Shipping label generator | POST /admin/labels/generate (filename field) | sink-predicate | A03:2021 | CWE-78 | `os-command-injection` |
| 10 | `blind-command-injection` | Blind OS command injection in connectivity check | OS Command Injection | ⑤ | corporate | Admin → Tools → Connectivity check | POST /admin/tools/ping (host field) | sink-predicate | A03:2021 | CWE-78 | `blind-command-injection` |
...
| 13 | `ldap-injection` | LDAP injection in corporate employee directory | LDAP Injection | ④ | corporate | Admin → Employees directory search | GET /admin/directory?u=<user> | canary | A03:2021 | CWE-90 | `ldap-injection` |
...
| # | ID | Title | Cat | Diff | Tier | GUI entry | Location | Oracle | OWASP | CWE | Skill |
|---|----|-------|-----|:----:|------|-----------|----------|--------|-------|-----|-------|
| 1 | `python-pickle-rce` | Python pickle deserialization RCE | Insecure Deserialization | ⑤ | corporate | Admin → Preferences → Import (base64 blob) | Cookie: prefs=<base64 pickle> / POST /admin/prefs/import | sink-predicate | A08:2021 | CWE-502 | `python-pickle-exploit` |
| 2 | `xxe-xml-import` | XXE in bulk product XML import | XXE | ④ | corporate | Admin → Inventory → Bulk import (XML) | POST /admin/inventory/import (XML body) | canary | A05:2021 A08:2021 | CWE-611 CWE-827 | `xxe-attacks` |
...
| 3 | `ssrf-blind-webhook` | Blind SSRF in webhook tester | SSRF | ⑤ | corporate | Admin → Integrations → Test a webhook | POST /admin/integrations/webhook/test | sink-predicate | A10:2021 API7:2023 | CWE-918 | `ssrf-blind` |
...
| 5 | `blind-xss-contact` | Blind stored XSS fired in the admin panel | Cross-Site Scripting | ④ | public | Support/Contact form → renders in Admin → Support Inbox | POST /contact -> rendered in /admin/support/inbox | sink-predicate | A03:2021 | CWE-79 | `blind-xss` |
...
| 3 | `file-inclusion-rce` | Local file inclusion to RCE via theme param | File Inclusion | ⑤ | corporate | Admin → Themes → Preview theme | GET /admin/render?theme=<name> | sink-predicate | A03:2021 | CWE-98 CWE-94 | `file-inclusion-rce` |
| 4 | `zip-slip-import` | Zip-slip path traversal in bulk import | File Upload | ⑤ | corporate | Admin → Inventory → Bulk import (ZIP) | POST /admin/inventory/import-zip | sink-predicate | A05:2021 | CWE-22 CWE-434 | `file-upload-abuse` |
...
| 2 | `default-creds-admin` | Default credentials on corporate admin | Default Credentials | ① | corporate | /admin (admin/admin) | POST /admin/login (admin/admin) | sink-predicate | A05:2021 A07:2021 | CWE-1392 CWE-1188 | `config-default-creds-exposed-panels` |
...
| 7 | `api-unsafe-consumption` | Unsafe consumption of third-party API data | Unsafe Consumption | ⑤ | corporate | Admin → Integrations → FX rate sync | POST /admin/integrations/fx-sync (upstream URL) | sink-predicate | A10:2021 API10:2023 | CWE-1104 CWE-345 | `api-unsafe-consumption` |
